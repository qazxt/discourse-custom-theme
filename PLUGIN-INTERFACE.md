...
| 主题文件 | 数据来源 | 渲染内容 |
|----------|----------|----------|
| `javascripts/robotime/connectors/above-site-header/robotime-header.hbs` | `nav_items`（由 JS 填充） | 自定义顶栏 DOM（Outlet：**above-site-header**，在默认站点 header **之上**） |
| `javascripts/robotime/connectors/below-site-header/robotime-carousel.hbs` | `hero_banners`（由 JS 填充） | 轮播区（Outlet：**below-site-header**，紧贴默认 header **之下**） |
